ActionController::Routing::Routes.draw do |map|
  map.resources :tenants, :only => [] do |tenant|
    ...
    tenant.resources :groups, :member => {:members => :get} do |group|
      group.resources :group_participations, :only => %w(new destroy), :collection => { :manage_members => :get, :manage_waiting_members => :get, :multi_create => :post }, :member => { :add_admin_control => :put, :remove_admin_control => :put, :approve => :put, :disapprove => :delete }
  ...
  map.resource :gadgets, :only => [], :member => {:gapps_calenders => :post, :gapps_mails => :post, :tenpo_notices => :post, :skip_questions => :post, :skip_notices => :post, :skip_recent_users => :post, :skip_recent_groups => :post, :infoscoop_header => :any, :skip_recents => :get}
